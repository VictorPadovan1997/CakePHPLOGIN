...
$detalhe = array();
foreach ($usuarios as $genero) {
    $editLink = $this->Js->link('Alterar', '/usuarios/edit/' . $genero['Usuario']['id'], array('update' => '#content'));
    $deleteLink = $this->Js->link('Excluir', '/usuarios/delete/' . $genero['Usuario']['id'], array('update' => '#content', 'confirm' => 'Tem certeza?'));
    $viewLink = $this->Js->link($genero['Usuario']['nome'], '/usuarios/view/' . $genero['Usuario']['id'], array('update' => '#content'));
    $detalhe[] = array(
        $viewLink, 
        $editLink . ' ' . $deleteLink
    );
